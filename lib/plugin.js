 ...
 */
function adjustTemplateIndentation(
  templateMarkdown,
  indentation,
  indentationType,
  adjustNormalTextLines
) {
  ...
  if (getLineIndentableType(firstLine) === indentationType) {
    ...
    // (stripping the indentation of the template so we don't have a double indentation)
  ...
  for (const line of lines) {
    ...
    if (line === "") {
      ...
      // If it's an indentable line, apply the indentation
    ...
    } else if (adjustNormalTextLines) {
      // If normal text should also be transformed and we are in an indentable block
      modifiedLines.push(`${indentation}${transformNormalText(line, lastIndentableType)}`);
    } else {
...
const plugin = {
  ...
  insertText: {
    ...
    async run(app) {
      ...
      // We need to use a workround of inserting a unique tag and then reading the note body to discover the line context
      ...
      // Check if the current line is indentable
      if (!isLineIndentable(currentLine)) {
        console.log("Current line is not indentable. Skipping template adjustment.");
      } else {
      ...
      let finalContent = thisNoteCurrentContent.replace(selectionTaggedUUID, templateMarkdown);
      ...
      thisNote.replaceContent(finalContent).then(function () {
        processTasks(app, newTaskUUIDs);
      });

      // const replacedSelection = await app.context.replaceSelection(templateMarkdown);

      // const testReplacement = await app.context.replaceSelection("Test");
