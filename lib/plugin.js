...
import embedHtml from "./embed.html" assert { type: "text" };
...
async function processInsertTemplate(
  app,
  thisNote,
  templateUUID,
...
async function findDefaultTagTemplate(app, tag) {
  ...
  if (tagDefaults[tag]) {
    ...
    const templateTag = app.settings[Settings.TEMPLATE_TAG] || "system/template";
...
const plugin = {
  ...
  appOption: {
    "Manage Defaults": {
      async run(app) {
        await app.openSidebarEmbed(0.5); // Adjust aspect ratio as needed
      },
    },
  },
  ...
  async onEmbedCall(app, action, ...args) {
    if (action === "getGlobalTemplate") {
      ...
      const settings = app.settings[Settings.TAG_DEFAULTS];
      let mappings = {};
      if (settings) {
        try {
          mappings = JSON.parse(settings);
        } catch (e) {
          // Handle parse error
          mappings = {};
        }
  ...
  dailyJotOption: {
    "There's a default dynamic template for this tag": {
      async check(app, noteHandle) {
        debugger;
        const tagTemplate = await findDefaultTagTemplate(app, noteHandle.tags[0]);
        debugger;
        if (tagTemplate) {
  ...
  noteOption: {
    "Set as Default Template": {
      async check(app, noteUUID) {
        ...
        return currentNote.tags.includes(app.settings[Settings.TEMPLATE_TAG] || "system/template");
      },
      run(app, noteUUID) {
  ...
  insertText: {
    ...
    "Pick Template": {
      ...
      async run(app) {
        ...
        const defaultUUID = findDefaultTemplate(app, templateNotes);
        ...
        const selection = await app.prompt("Pick a template", {
          inputs: [{ type: "select", options, value: defaultUUID }],
        });
        ...
        const thisNote = await app.notes.find(app.context.noteUUID);
        return processInsertTemplate(app, thisNote, selection, {
          smartIndentation: true,
        });
      },
    },
