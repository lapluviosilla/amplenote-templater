...
function findDefaultTemplate(app, templateNotes) {
  app.settings["defaults"] ||= {};
  return app.settings[Settings.DEFAULT_TEMPLATE];
}

async function processInsertTemplate(app, templateUUID) {
  ...
  insertText: {
    "Tag Default Template": {
      async check(app) {
        ...
        const defaultTemplate = templateNotes.find((template) =>
          template.tags.some((t) => thisNote.tags.includes(t))
        );
      ...
      async run(app) {
        return processInsertTemplate(app, findDefaultTemplate(app, null));
      },
    },
    "Global Default Template": {
      async check(app) {
        const defaultId = findDefaultTemplate(app, null);
        if (defaultId) {
    ...
    "Pick Template": {
      ...
      async run(app) {
        ...
        return processInsertTemplate(app, selection);
      },
    },
  },
};
