 ...
 * @param {Boolean} includeHeader if we want the inner section content or including the header
 * @returns String of section
 */
function extractSection(markdown, sectionName, sectionInner = false) {
  // Match the section header
  const headerRegex = new RegExp(`^(#{1,6})\\s+${sectionName}\\s*$`, "m");
  ...
  // Regex to find the next header of the same or higher level or a horizontal rule
  const endRegex = new RegExp(`(^#{1,${headerLevel}}\\s+.*$)|(^---+$)`, "m");
  const restOfMarkdown = markdown.slice(headerEndIndex);
  const endMatch = endRegex.exec(restOfMarkdown);

  let endIndex;
  if (endMatch) {
    endIndex = endMatch.index;
  } else {
    endIndex = restOfMarkdown.length;
  }

  // Extract the section content based on the includeHeader flag
  const sectionContent = sectionInner
    ? markdown.slice(headerEndIndex, headerEndIndex + endIndex)
    : markdown.slice(headerStartIndex, headerEndIndex + endIndex);
...
function createFootnote(result, expression, footnoteData) {
  ...
  footnoteData.content.push(
    `[^templater-${footnoteData.prefix}-${footnoteIndex}]: [${result}]()\n${expression}`
  );
  return `[${result}][^templater-${footnoteData.prefix}-${footnoteIndex}]`;
}
 ...
 * @returns {Promise<Object{markdown, linkedTemplateFoonotes, linkedTemplateTaskUUIDs}>} - The processed markdown, link footnotes and link tasks
 */
async function processLinks(app, markdown) {
  // Updated regex to capture optional flag ("?", "_", "=", "#>", "#"), link content, section, and alias
  const linkRegex = /\\\[\\\[(\??_?=?)?([^\]#|]+?)(?:#(>?)([^\]|]+))?(?:\\\|([^\]]+))?\\\]\\\]/g;
  // Match dynamic template expressions too as an alias
  const expressionRegex = /\{(=)([^\]#|]+?)(?:#(>?)([^\]|]+))?\}/g;
  const matches = [...markdown.matchAll(linkRegex), ...markdown.matchAll(expressionRegex)];

  const linkedTemplateTaskUUIDs = [];
  const linkedTemplateFootnotes = [];
  ...
  for (const match of matches) {
    ...
    const isSilent = match[1] === "_" ? true : false; // Check if there's a silent flag
    const isTemplate = match[1] === "=" ? true : false; // Handle template insertion case
    let linkContent = match[2]; // e.g., "Note Name" or "tag/Note Name"
    const sectionType = match[3]; // ">" for content without header or "" for with header
    let section = match[4]; // e.g., "#Section" (optional)
    const alias = match[5]; // e.g., "Display Name" (optional)
    ...
    let noteHandle;
    if (isOptional || isTemplate || isSilent) {
      // Handle the "optional" case "?" and template insertion
      noteHandle = tag
        ? await app.notes.find({ name: noteName, tags: [tag] })
        : await app.notes.find({ name: noteName });
    } else {
    ...
    if (!noteHandle && isSilent) {
      // Remove the link entirely
      markdown = markdown.replace(fullMatch, "");
      continue;
    }
    if (!noteHandle && isTemplate) {
      // If the note doesn't exist and is a template, just leave as is
      continue;
    }

    // Handle template insertion vs normal linking
    if (isTemplate) {
      let templateContent = await noteHandle.content();
      if (section) {
        templateContent = extractSection(templateContent, section, sectionType === ">");
      }

      // It's a template!
      const { templateMarkdown, footnotes, newTaskUUIDs } = await processTemplate(
        app,
        noteHandle.uuid,
        templateContent
      );
      linkedTemplateTaskUUIDs.push(...newTaskUUIDs);
      linkedTemplateFootnotes.push(...footnotes);
      markdown = smartTemplateInsert(markdown, templateMarkdown, fullMatch);
      // markdown = markdown.replace(fullMatch, templateMarkdown);
      continue;
    } else {
      // It's a normal link
  ...
  return { markdown, linkedTemplateFootnotes, linkedTemplateTaskUUIDs };
}
...
function replaceTaskUUIDs(markdown) {
  ...
  const newUUIDs = []; // Array to store new UUIDs
  let match;
  ...
  return { modifiedMarkdown, newUUIDs }; // Return the modified markdown and new UUIDs
}
...
function smartTemplateInsert(baseContent, templateMarkdown, insertionTag) {
  // Find the position of the selection in the current content
  const insertPosition = baseContent.indexOf(insertionTag);
  if (insertPosition === -1) {
  ...
  const currentLine = getCurrentLine(baseContent, insertPosition);
  ...
  const insertedMarkdown = baseContent.replace(insertionTag, templateMarkdown);
...
/**
 *
 * @param {*} app app reference for processing link and tasks
 * @param {*} templateUuid uuid of the template being inserted
 * @param {*} templateMarkdown template content
 * @returns {templateMarkdown: String, footnoteData: Object, newTaskUUIDs: Array[String]} Returns processed template and new task uuids for post-processing
 */
async function processTemplate(app, templateUuid, templateMarkdown) {
  // Initialize footnote counter and storage
  // We use a 8 character prefix from the uuid to distinguish footnotes between nested templates
  const footnoteData = { prefix: templateUuid.slice(0, 8), counter: 1, content: [] }; // Wrap counter in an object
  ...
  // Step 3: Process Note Links ([[...]]) and any nested templates
  const {
    markdown: linksMarkdown,
    linkedTemplateFootnotes,
    linkedTemplateTaskUUIDs,
  } = await processLinks(app, templateMarkdown);

  // Step 4: Pre-process Tasks
  // Replace task UUIDs with new ones in the template markdown
  const { modifiedMarkdown, newUUIDs } = replaceTaskUUIDs(linksMarkdown);
  templateMarkdown = modifiedMarkdown;

  const footnotes = footnoteData.content.concat(linkedTemplateFootnotes);
  const newTaskUUIDs = linkedTemplateTaskUUIDs.concat(newUUIDs);

  // Step 5: Append Footnotes
  // if (footnoteData.con tent.length > 0) {
  //   templateMarkdown += "\n\n" + footnoteData.content.join("\n");
  // }

  return { templateMarkdown, footnotes, newTaskUUIDs };
}
 ...
 * - section: Specify a section of the template to use
 * - sectionInner: Whether we just want the inner section content or not
...
export async function processInsertTemplate(
  ...
  thisNote,
  template,
  { insertMode = "smart", section = null, sectionInner = false } = {}
) {
  // const thisNote = await app.notes.find(thisNoteUUID);
  let templateBaseContent = await template.content();
  if (section && section !== "") {
    templateBaseContent = extractSection(templateBaseContent, section, sectionInner);
  }
  let { templateMarkdown, footnotes, newTaskUUIDs } = await processTemplate(
    app,
    template.uuid,
    templateBaseContent
  );

  if (footnotes.length > 0) templateMarkdown += "\n\n" + footnotes.join("\n");
  ...
  if (insertMode === "smart" && app.context && app.context.replaceSelection) {
    ...
    const finalContent = smartTemplateInsert(
      thisNoteCurrentContent,
      templateMarkdown,
      selectionTaggedUUID
    );
...
async function createNoteFromLink(app, createNoteParams) {
  const evaluatedName = createNoteParams.name ? evaluateExpressions(createNoteParams.name) : null;
  ...
  if (templateNote) {
    await processInsertTemplate(app, newNote, templateNote, {
      insertMode: "replace",
      section: createNoteParams.section,
      sectionInner: createNoteParams.sectionInner || false,
    });
  }
...
const plugin = {
  ...
  linkOption: {
    ...
    "Edit Link": {
      check(app, link) {
        ...
        // Some platforms/browsers parse it as hostname, and others as pathname
        const pluginMatch =
          parsedUrl.hostname !== ""
            ? parsedUrl.hostname === app.context.pluginUUID
      ...
      async run(app, link) {
        ...
        const result = await app.prompt("Edit Dynamic New Note Link", {
          inputs: [
            ...
            {
              label: "Convert to Note Link After Creation",
              type: "checkbox",
              value: !!paramsObject.convertToLink,
            },
            { label: "Section", type: "string", value: paramsObject.section },
            { label: "Skip Section Header?", type: "checkbox", value: !!paramsObject.sectionInner },
          ],
        });

        if (result) {
          let [name, tags, source, convertToLink, section, sectionInner] = result;
          source = source ? source.uuid : null;
          let newParams = Object.entries({ name, tags, source, section }).reduce((acc, [k, v]) => {
            if (v && v !== "") {
              acc[k] = v;
            } else {
              delete acc[k];
          ...
          if (section && sectionInner) {
            newParams.sectionInner = true;
          } else {
            delete newParams.sectionInner;
          }
          if (convertToLink) {
            newParams.convertToLink = true;
          } else {
            delete newParams.convertToLink;
          }
          const finalParams = { action: "create", ...newParams };
          parsedUrl.search = new URLSearchParams(finalParams).toString();
  ...
  dailyJotOption: {
    "There's a default dynamic template for this tag": {
      ...
      async run(app, noteHandle) {
        ...
        // If it's a new jot the note doesn't exist. Create it
        if (noteHandle.uuid === null) {
          note = await app.notes.create(noteHandle.name, noteHandle.tags);
        } else {
          // If note does exist then find it
          note = await app.notes.find(noteHandle);
        }
    ...
    "Tag Default Template": {
      ...
      async run(app) {
        ...
        if (tagDefaultTemplate) {
          ...
          console.warn("Template disappeared before we tried to insert");
        }
      },
    },
      ...
      async run(app) {
        const result = await app.prompt("Create a Dynamic New Note Link", {
          inputs: [
            ...
            { label: "Convert to Note Link After Creation", type: "checkbox" },
          ],
          actions: [{ label: "Pick Section", icon: "subheader", value: "pick-section" }],
        });
        const [title, name, tags, templateHandle, convertToLink, action] = result;

        if (result) {
          let section;
          let sectionInner;
          if (action === "pick-section") {
            const sections = await app.getNoteSections({ uuid: templateHandle.uuid });

            // Turn the sections into a select option list
            const sectionOptions = sections.reduce((acc, section) => {
              if (section.heading && !section.index) {
                acc.push({
                  label: `h${section.heading.level} ${section.heading.text}`,
                  value: section.heading.text,
                });
              }
              return acc;
            }, []);
            const pickedSection = await app.prompt("Pick a section", {
              inputs: [
                { label: "Section", type: "select", options: sectionOptions },
                { label: "Skip Header? (Only insert inner content)", type: "checkbox" },
              ],
            });
            ...
            [section, sectionInner] = pickedSection;
          }

          const params = {
            action: "create",
            name,
            source: templateHandle ? templateHandle.uuid : "",
          };
          if (tags) params.tags = tags;
          if (section && section !== "") params.section = section;
          if (section && sectionInner) params.sectionInner = true;
          if (convertToLink) params.convertToLink = true;

          const query = new URLSearchParams(params).toString();

          await app.context.replaceSelection(
            `[${title}](plugin://${app.context.pluginUUID}?${query})`
          );
        }
        return null;
      },
  ...
  async linkTarget(app, query = null) {
    ...
    if (paramsObject.action === "create") {
      ...
      const newNote = await createNoteFromLink(app, paramsObject);

      // If it's a single use link then convert it
      if (newNote && paramsObject.convertToLink) {
        const newNoteUrl = await newNote.url();
        const thisNoteContent = await app.getNoteContent({ uuid: app.context.noteUUID });

        // Build regexp to match plugin link and replace with note link
        const pluginUrl = `plugin://${app.context.pluginUUID}?${query}`;
        const linkMatcher = new RegExp(`\\[.*\\]\\(${escapeRegex(pluginUrl)}\\)`);

        // Replace it
        const editedContent = thisNoteContent.replace(
          linkMatcher,
          `[${newNote.name}](${newNoteUrl})`
        );
        await app.replaceNoteContent({ uuid: app.context.noteUUID }, editedContent);
      }
    }
  },
};
