  ...
  dailyJotOption: {
    "There's a default dynamic template for this tag": {
      async check(app, noteHandle) {
        ...
        const suggestionsGiven = expiryStorage.getItem("jotSuggestionGiven-" + dailyJotTag) || {};

        if (tagTemplate && !suggestionsGiven[dailyJotTag]) {
      ...
      async run(app, noteHandle) {
        ...
        expiryStorage.setItem("jotSuggestionGiven-" + dailyJotTag, true, startOfTomorrow());
