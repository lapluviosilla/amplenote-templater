...
function parseDateExpression(expr) {
  ...
  if (lowerExpr === "tomorrow") {
    return { success: true, date: addDays(now, 1) };
  }
  if (lowerExpr === "yesterday") {
    return { success: true, date: subDays(now, 1) };
  }
...
function parseAbsoluteDate(expr, referenceDate) {
  ...
  // 6. Handle weekdays like "Monday", "Tuesday", etc.
  if (weekdays.includes(trimmed)) {
    const weekdayIndex = getWeekdayIndex(trimmed);
    const nextWkday = nextWeekday(referenceDate, weekdayIndex);
    // Set time to start of day
    return set(nextWkday, {
      hours: 0,
      minutes: 0,
      seconds: 0,
      milliseconds: 0,
...
function parseRelativeDate(expr, referenceDate) {
  ...
  const inMatch = trimmed.match(
    /^in\s+(\d+|\w+)\s+(day|week|month|year|hour|minute|second)s?$/i
  );
  if (inMatch) {
    const value = parseInt(inMatch[1], 10) || wordToNumber(inMatch[1]);
    const unit = inMatch[2].toLowerCase();
    if (isNaN(value)) return null;
    return addTime(referenceDate, value, unit);
  }

  // 4. Handle "A month ago", "2 days ago", etc.
  const agoMatch = trimmed.match(
    /^(\d+|\w+)\s+(day|week|month|year|hour|minute|second)s?\s+ago$/i
  );
  if (agoMatch) {
...
function parseTimeExpression(expr, referenceDate) {
  ...
  if (timeMatch) {
    ...
    let dateWithTime = setHours(referenceDate, hours);
 ...
 * @param {string} month
 * @returns {number} - 0-based month index or -1
 */
 ...
 * @param {string} day
 * @returns {number} - 0 (Sunday) to 6 (Saturday) or -1
 */
...
function nextWeekday(referenceDate, weekday) {
  ...
  date.setDate(date.getDate() + ((7 + weekday - date.getDay()) % 7 || 7));
  return date;
}
 ...
 * @param {string} word - The word representation of the number.
