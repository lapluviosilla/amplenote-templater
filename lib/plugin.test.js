...
describe("This here plugin", () => {
  ...
  describe("Plugin Note Create Link", () => {
    ...
    test("It should insert a valid link with a template", async () => {
      ...
      expect(app.notes.create).toHaveBeenCalledWith(null, null);
      const storedNotes = app._storedNotes;
      expect(storedNotes[storedNotes.length - 1].body).toEqual(`A template [12][^templater-uuid-1-1]

[^templater-uuid-1-1]: [12]()
3+9`);
    });
  ...
  describe("dailyJotOption", () => {
    ...
    test("Should insert when daily jot option suggestion is clicked", async () => {
      ...
      expect(note.body).toEqual(`- Test Daily Jot Template [400][^templater-template-1]

[^templater-template-1]: [400]()
20*20
Hello`);
    });
    test("Should create the daily jot note if new when suggestion is clicked", async () => {
      const templateNote = mockNote("- Test Daily Jot Template", "Template", "template-2");
      app._storedNotes.push(templateNote);
      app.context.url = "https://www.amplenote.com/notes/jots?tag=daily-jots";
      // Make the template note the default template for daily jots
      app.settings["Tag Default Templates"] = '{"daily-jots":"template-2"}';

      await plugin.dailyJotOption["There's a default dynamic template for this tag"].run(app, {
        uuid: null,
        name: "New Jot",
        tags: "daily-jots",
      });
      ...
      const newNote = app._storedNotes[app._storedNotes.length - 1];
      expect(newNote.name).toEqual("New Jot");
      expect(newNote.tags).toEqual("daily-jots");
    });
  });
...
`;
    ...
    test("Should process nested template sections", async () => {
      const nestedTemplateContent = `# First Section
Nested Content in First Section
## Subsection
Subsection Content
# Second Section
Text we don't want`;
      const nestedTemplateNote = mockNote(
        nestedTemplateContent,
        "Nested Template",
        "template-uuid-1"
      );
      const templateContent = `\\[\\[=Nested Template#First Section\\]\\]`;
      const templateNote = mockNote(templateContent, "Template", "template-uuid-2");

      const targetContent = `{Templater}`;
      const { app, note } = mockAppWithContent(targetContent);
      app._storedNotes.push(nestedTemplateNote);
      app._storedNotes.push(templateNote);
      ...
      const expectedMarkdown = `# First Section
Nested Content in First Section
## Subsection
Subsection Content`;
    ...
    test("Should process nested template sections excluding header", async () => {
      const nestedTemplateContent = `# First Section
Nested Content in First Section
## Subsection
Subsection Content
...
Text we don't want`;
      ...
      const expectedMarkdown = `Nested Content in First Section
## Subsection
Subsection Content`;
...
Here is a template insertion: \\[\\[=system/template/Tidbit\\]\\]
Here is a non-existant template insertion: \\[\\[=system/template/No Note\\]\\]
...
Here is a silent link to a non-existent note: \\[\\[_projects/NotARealProject\\]\\]
`;
      ...
      const templateNote = mockNote(templateContent, "Sample Template", "temp-1", [
        "system/template",
      ]);
      app._storedNotes.push(templateNote);

      const nestedTemplateNote = mockNote("15 * 34 = {15*34}", "Tidbit", "temp-2", [
        "system/template",
      ]);
      app._storedNotes.push(nestedTemplateNote);
...
[^templater-temp-1-1]: [${expectedToday}]()
Today
[^templater-temp-1-2]: [${expectedNow}]()
Now
[^templater-temp-1-3]: [${expectedMath}]()
2+2
[^templater-temp-1-4]: [${expectedMathPi}]()
2*pi
[^templater-temp-1-5]: [${expectedMonday}]()
Monday at 3pm
[^templater-temp-1-6]: [04-29-2024]()
"MM-dd-yyyy":In two days
[^templater-temp-2-1]: [510]()
15*34
`.trim();
      ...
      //
      const expectedMarkdown = `
Today's date is [${expectedToday}][^templater-temp-1-1].
Now is [${expectedNow}][^templater-temp-1-2].
2 + 2 equals [${expectedMath}][^templater-temp-1-3].
2 * pi equals [${expectedMathPi}][^templater-temp-1-4].
Meeting on [${expectedMonday}][^templater-temp-1-5].
Another Meeting on [04-29-2024][^templater-temp-1-6].
Invalid expression: {Febtember 10th}.
...
Check out your daily note for tomorrow: [${expectedTomorrow}#My Tasks](https://www.amplenote.com/notes/uuid-4#My_Tasks)
Check out your special project note: [Project 7](https://www.amplenote.com/notes/uuid-5)
Here is the project note again: [Project 7](https://www.amplenote.com/notes/uuid-5)
Here is a template insertion: 15 * 34 = [510][^templater-temp-2-1]
Here is a non-existant template insertion: \\[\\[=system/template/No Note\\]\\]
Here it is in a different tag (new note): [Project 7](https://www.amplenote.com/notes/uuid-6)
Here is a link with a different display name: [The Special Project](https://www.amplenote.com/notes/uuid-6#Hello)
Here is a optional dynamic link to a non-existent note: [[projects/NotARealProject]]
Here is a silent link to a non-existent note: 
...
`.trim();
      ...
      // There should be 6 notes (the seed note, two template notes, and the three notes created)
      expect(app.notes.create).toHaveBeenCalledTimes(3);
      expect(app._storedNotes.length).toBe(6);
