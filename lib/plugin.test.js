...
Invalid expression: {Febtember 10th}.
- [ ] Follow up {start:Tomorrow} at the latest<!-- {"uuid":"task-1"} -->
- [ ] Hide 7 til {hide:Thursday} please and start by {start:Friday}<!-- {"uuid":"task-2"} -->
...
`.trim();
      ...
      expect(app.updateTask).toHaveBeenNthCalledWith(1, "task-1", {
        content: "Follow up Tomorrow at the latest",
        startAt: expectedTomorrowTime,
      });
      expect(app.updateTask).toHaveBeenNthCalledWith(2, "task-2", {
